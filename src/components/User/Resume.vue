    ...
    <div class="user-resume">
        <div class="user-resume__content">
            <UserResumeEntry 
                ...
                :title="t('page.account.resume.read')" :value="map?.read?.stories?.length || 0" />
            <UserResumeEntry 
                @click="setActiveSection('pending')"
                :active="activeSection === 'pending'" 
                :title="t('page.account.resume.pending')" :value="map?.pending?.stories?.length || 0"/>
            <UserResumeEntry 
                @click="setActiveSection('favorites')"
                :active="activeSection === 'favorites'" 
                :title="t('page.account.resume.favorites')" :value="map?.favorites?.stories?.length || 0" />
        </div>
