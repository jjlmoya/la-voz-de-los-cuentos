...
function checkSagaAchievements(): void {
  try {
    ...
    // Usar el conteo total de historias completadas como proxy para sagas
    // (simplificado: muestra progreso basado en total de historias)
    const sagaAchievements = ALL_ACHIEVEMENT_DEFINITIONS.filter(d => d.type === 'saga')

    sagaAchievements.forEach(definition => {
      const threshold = definition.threshold || 1
      const shouldUnlock = completedCount >= threshold
      ...
      // Mostrar progreso
      updateAchievementProgress(definition.id, Math.min(completedCount, threshold), threshold)
    })
  } catch (error) {
...
function checkCharacterAchievements(): void {
  try {
    ...
    // Usar como proxy del conteo de personajes
